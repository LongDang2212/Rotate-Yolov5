...
namespace ryolo
{
  ...
  Engine::Engine(const char *onnx_model, size_t onnx_size, const vector<int> &dynamic_batch_opts,
    float score_thresh, int top_n, const vector<vector<float>> &anchors, const vector<float> &strides, // for decode
    ...
    bool verbose, size_t workspace_size)
  {
    ...
    for (int i = 0; i < nbOutputs; i++)
    {
      ...
      auto decodePlugin = DecodePlugin(score_thresh, top_n, strides[i], anchors[i]);
      ...
      classes.push_back(layer_decode->getOutput(2));
    }
    ...
    auto layer = network->addPluginV2(concat.data(), concat.size(), nmsRotatePlugin);
