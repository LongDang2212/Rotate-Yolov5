...
from utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
    ...
    rotate_non_max_suppression, rotate_scale_coords
...
@torch.no_grad()
def detect(weights='rotate-yolov5s-ucas.pt',  # model.pt path(s)
    ...
    # Rotate does not support second-stage classifier
    classify = False
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain xywh

            assert not save_crop, "rotate does not support save_crop"
            if len(det):
                # Rescale boxes from img_size to im0 size
                det[:, :4] = rotate_scale_coords(img.shape[2:], det[:, :4], im0.shape).round()
                # Print results
            ...
            # Print time (inference + rotate NMS)
